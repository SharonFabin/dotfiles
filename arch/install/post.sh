...
function post {
	...
	echo -e "  ${YELLOW}4.${RESET} Backups: set up with timeshift-gtk"
	echo -e "  ${YELLOW}5.${RESET} Sound issues: install sof-firmware"
	echo -e "  ${YELLOW}6.${RESET} Mouse lag: add usbcore.autosuspend=0 to /etc/default/grub"
	echo -e "  ${YELLOW}7.${RESET} Firewall: configure with gufw"
	echo -e "  ${YELLOW}8.${RESET} GTK theme: configure with nwg-look"
}
